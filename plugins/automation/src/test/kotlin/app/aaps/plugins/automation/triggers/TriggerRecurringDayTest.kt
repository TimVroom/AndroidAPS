...
class TriggerRecurringDayTest : TriggerTestBase() {

    @Test fun shouldRunTest() {
        var t = TriggerRecurringDay(injector)
        t.days[WeekDay.DayOfWeek.MONDAY] = true
        assertThat(t.shouldRun()).isFalse()

        // scheduled 1 min before
        t = TriggerRecurringDay(injector)
    ...
    @Test
    fun toJSONTest() {
        val t = TriggerRecurringDay(injector)
        t.days[WeekDay.DayOfWeek.SUNDAY] = true
    ...
    fun fromJSONTest() {
        val t = TriggerRecurringDay(injector)
        val t2 = TriggerDummy(injector).instantiate(JSONObject(t.toJSON())) as TriggerRecurringDay
