...
class TriggerRecurringDay(injector: HasAndroidInjector) : Trigger(injector) {

    val days = InputWeekDay()

    constructor(injector: HasAndroidInjector, triggerRecurringDay: TriggerRecurringDay) : this(injector) {
        if (days.weekdays.size >= 0)
            System.arraycopy(triggerRecurringDay.days.weekdays, 0, days.weekdays, 0, triggerRecurringDay.days.weekdays.size)
    }
    ...
    override fun friendlyName(): Int = R.string.recurringDay
    ...
    override fun duplicate(): Trigger = TriggerRecurringDay(injector, this)

    override fun generateDialog(root: LinearLayout) {
        LayoutBuilder()
            .add(StaticLabel(rh, R.string.recurringDay, this))
            .add(days)
            .build(root)
    }
}